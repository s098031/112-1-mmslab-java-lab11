...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_insert.setOnClickListener(view -> {
            if(ed_book.length()<1||ed_price.length()<1) Toast.makeText(MainActivity.this,"欄位請誤留空",Toast.LENGTH_SHORT).show();
            else {
                try{
        ...
        btn_delete.setOnClickListener(view -> {
            if(ed_book.length()<1) Toast.makeText(MainActivity.this,"書名請誤留空",Toast.LENGTH_SHORT).show();
            else {
                try{
